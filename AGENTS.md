...
AGENTS.md in the same commit.** It is the substitute for re-reading the project every session.
...
UI verification: use the preview tools with `.Codex/launch.json` server **"texthooker-verify"
...
- **ocr.py** — OCR fallback for unhookable games: tkinter drag-a-box region picker (run as a
  ...
  caches complete frames by pixel hash so stability confirmation is cheap. Thresholds mirror
  Meikipop (det 0.5, rec 0.1, punctuation factor 0.2); initialization/download failure falls
  through without breaking OCR. Otherwise engine = `HybridOcr` when manga-ocr is installed:
  Windows.Media.Ocr (persistent PowerShell worker) locates text —
